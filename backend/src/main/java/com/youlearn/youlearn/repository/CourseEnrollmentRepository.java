...
public interface CourseEnrollmentRepository extends JpaRepository<CourseEnrollment, Long> {
    ...
    @Modifying
    @Query("update CourseEnrollment ce set ce.isCertificationObtained = ?1, ce.completedAt = current_timestamp, ce.isGradeSubmitted = true where ce.user.id = ?2 and ce.course.id = ?3")
